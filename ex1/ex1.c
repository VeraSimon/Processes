...
Starts at the value set pre-fork, then updates to the child set value for
the remainder of the lifetime of the child process.
What happens to the variable when both the child and parent change the value
of x?
The child-set value remains that way for the lifetime of the child, and the
parent-set value remains that way for the lifetime of the parent. They're
mutually exclusive that way.
~~ Output ~~
$ ./ex1
